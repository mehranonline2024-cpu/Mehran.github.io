...
async function changeOrder(sb: ReturnType<typeof adminClient>, order: any, patch: Record<string, unknown>) {
  const { data, error } = await sb.from('orders').update(patch).eq('id', order.id).eq('status', order.status).select('id').maybeSingle()
  if (error) throw error
  if (!data) throw new Error('De bestelling is intussen gewijzigd. Vernieuw en probeer opnieuw.')
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'accept') {
      ...
      await changeOrder(sb, order, { status: 'accepted', accepted_at: new Date().toISOString(), accepted_by: user.id })
    ...
    if (action === 'reject') {
      if (!['new', 'rejected'].includes(order.status)) return json(req, { error: 'Alleen een nieuwe bestelling kan worden geweigerd. Wacht bij online bestellingen eerst op de betaling.' }, 409)
      if (order.payment_method === 'online' && !['paid', 'refunded'].includes(order.payment_status)) return json(req, { error: 'Online betaling is nog niet bevestigd.' }, 409)
      // Claim rejection before contacting Stripe, so acceptance cannot race a refund.
      if (order.status === 'new') {
        await changeOrder(sb, order, { status: 'rejected', rejected_at: new Date().toISOString(), accepted_by: user.id, rejection_reason: String(body.reason || '').trim().slice(0, 250) || null })
      }
      if (order.payment_method !== 'online' || order.payment_status === 'refunded') return json(req, { ok: true, status: 'rejected', refunded: order.payment_status === 'refunded' })
      try {
        if (!stripeKey) throw new Error('Stripe is niet beschikbaar.')
        ...
        if (!paymentIntent) throw new Error('Betaling niet gevonden.')
        const refund = order.stripe_refund_id
          ? await stripe.refunds.retrieve(order.stripe_refund_id)
          : await stripe.refunds.create({ payment_intent: paymentIntent }, { idempotencyKey: `grill-time-reject-${id}` })
        const succeeded = refund.status === 'succeeded'
        const { error } = await sb.from('orders').update({ stripe_refund_id: refund.id, ...(succeeded ? { payment_status: 'refunded' } : {}) }).eq('id', id).eq('status', 'rejected')
        if (error) throw error
        if (['failed', 'canceled'].includes(String(refund.status))) throw new Error('Controleer de terugbetaling in Stripe.')
        return json(req, { ok: true, status: 'rejected', refunded: succeeded, refundPending: !succeeded })
      } catch (_) {
        return json(req, { error: 'Bestelling geweigerd, maar terugbetaling nog niet bevestigd. Gebruik Terugbetaling controleren en controleer Stripe.', refundNeedsAttention: true }, 503)
      }
    }

    if (['preparing', 'ready', 'on_way', 'completed'].includes(action) && order.payment_method === 'online' && order.payment_status !== 'paid') return json(req, { error: 'Online betaling is nog niet bevestigd.' }, 409)
    ...
    if (action === 'on_way') {
      ...
      await changeOrder(sb, order, { status: 'on_way', on_way_at: new Date().toISOString(), accepted_by: user.id })
    ...
    if (action === 'completed') {
      ...
      await changeOrder(sb, order, { status: 'completed', completed_at: new Date().toISOString(), accepted_by: user.id })
    ...
    if (action === 'preparing' || action === 'ready') {
      ...
      await changeOrder(sb, order, { status: action, accepted_by: user.id })
      return json(req, { ok: true, status: action })
    }

    if (action === 'cash_paid') {
      if (order.payment_method !== 'cash' || ['rejected', 'cancelled'].includes(order.status)) return json(req, { error: 'Dit is geen cashbestelling.' }, 409)
