...
async function checkout({paused=[false],type='delivery',payment='cash',get=false,statusError=false}={}){
 ...
 const db={from(table){let mutation=null,payload;const q={};for(const method of ['select','eq','gte','in','single'])q[method]=()=>q;
  ...
  q.then=(resolve,reject)=>{let result={data:[],error:null,count:0};
   ...
   else if(table==='store_settings'){result=statusError?{error:new Error('unavailable')}:{data:{delivery_paused:paused[Math.min(reads,paused.length-1)]},error:null};reads++;}
 ...
 const req=new Request('https://example.test',{method:get?'GET':'POST',headers:{origin:'https://grilltime.be','content-type':'application/json'},...(!get?{body:JSON.stringify({paymentMethod:payment,orderType:type,items:[{productId:'pizza',qty:1}],customer:{name:'Test',phone:'0490000001'},address:'Teststraat 1',city:'8400 Oostende'})}:{})});
...
(async()=>{
 for(const paused of [false,true]){const r=await checkout({get:true,paused:[paused]});assert.equal(r.status,200);assert.deepEqual(r.body,{deliveryPaused:paused});assert.equal(r.writes.length,0);}
 ...
 const unavailable=await checkout({statusError:true});assert.equal(unavailable.status,500);assert.equal(unavailable.writes.length,0);
 for(const payment of ['cash','online']){const pickup=await checkout({type:'pickup',paused:[true],statusError:true,payment});assert.equal(pickup.status,200);assert.equal(pickup.reads,0);assert.equal(pickup.body.deliveryFeeCents,0);assert.equal(pickup.writes.filter(w=>w.table==='orders'&&w.mutation==='insert').length,1);}
